...
void XlaCompileOp::Compute(OpKernelContext* ctx) {
  ...
  if (GetXlaOpsCommonFlags()->tf_xla_always_defer_compilation ||
    ...
    const Status status = CompileToLocalExecutable(
        ...
        /*may_alias_resource_update=*/true, &client, &kernel, &executable);
...
void XlaRunOp::Compute(OpKernelContext* ctx) {
  ...
  std::map<int, const Tensor*> snapshot_or_var_tensors;
  {
    ...
    for (auto& [variable_index, variable_tensor] :
         closure.resource_var_snapshots()) {
      const Tensor* tensor_ptr = nullptr;
      if (variable_tensor.has_value()) {
        if (resource_var_ptrs.count(variable_index) &&
            resource_var_ptrs[variable_index]->SharesBufferWith(variable_tensor.value())) {
          ...
          variable_tensor.reset();
          tensor_ptr = resource_var_ptrs[variable_index];
        } else {
          // Otherwise, Use the snapshot.
          tensor_ptr = &variable_tensor.value();
        }
      }
      snapshot_or_var_tensors.emplace(variable_index, tensor_ptr);
    }
