...
PersistentCompilationCache::PersistentCompilationCache()
{
  ...
  for (const auto& file : files) {
    ...
    std::string text;
    std::string fullpath = tsl::io::JoinPath(cache_dir_, file);
    if (!tsl::ReadFileToString(env, fullpath, &text).ok()) {
...
int64_t PersistentCompilationCache::CreateKey(
  ...
  std::string llvm_str = llvm_ir::DumpToString(llvm_module);
  ...
  int64_t key = tsl::Hash64(llvm_str);
  key = tsl::Hash64Combine(key, compute_capability.major);
  key = tsl::Hash64Combine(key, compute_capability.minor);
  key = tsl::Hash64Combine(key, tsl::Hash64(ptx_options));
...
void PersistentCompilationCache::AddToCache(int64_t key, absl::string_view text,
  ...
  if (!env->CreateUniqueFileName(&text_tmp, "")) {
    ...
    if (!tsl::WriteStringToFile(env, text_tmp, text).ok()) {
      ...
      std::string text_file = tsl::io::JoinPath(cache_dir_, key_str);
...
void PersistentCompilationCache::AddToCache(int64_t key, const std::string &ptx) {
  int64_t ptx_key = tsl::Hash64Combine(key, kPtxHash);
